...
## MAIN  
        my @PROC=`ps -ef | grep \"perl ./gemm_insert.pl\"`;
        if ($#PROC>2) {die "DIE: GEMM Process already started: @PROC\n";}
#die "didn't die @PROC\n";
...
sub parse_vcf {
   ...
   if ($file_info->{'tissue'} eq "germline") {$var_info{'aberration_filter'}='PASS';}
...
sub assign_aberration {
  ...
  if (ref($var_info_ref) eq "HASH") {
    if ($var_info_ref->{'alt'} eq "<SV>") {
      ...
      if (abs($var_info_ref->{'qual'})<0.75) {$var_info_ref->{'aberration_filter'}='FAIL'}
      ...
      if ($var_info_ref->{'functional_change'}=~/(CODON)|(FRAME)|(SPLICE)|(START_)|(STOP_)|(NON_SYNONYMOUS)/) { $var_info_ref->{'aberration_filter'}='PASS'; } else { $var_info_ref->{'aberration_filter'}='NON_GENIC'; }
...
sub indexing {
  my $SNVS = $_[0];
	...
	'gene','count','total_count','KGPhase1','OTHERKG','ac','af','baseranksum','clinvar','codon_change','coordinate','count','total_count',
